export class PomodoroTimer {
    ...
    private _settingsData!: {focusTime: number, shortBreak: number, longBreak: number, longBreakInterval: number }
    ...
    private getNextTimerMinutes(focusCount: number): number {
        ...
        if (this._isFocused) {
            ...
            return (focusCount % this._settingsData.longBreakInterval) === 0 ? this._settingsData.longBreak : this._settingsData.shortBreak;
        }
    }
    ...
    public loadSettings() {
        if (localStorage.getItem(this.PomodoroSettingsLocalStorageKey)) {
            this._settingsData = JSON.parse(localStorage.getItem(this.PomodoroSettingsLocalStorageKey)!);
            this._currentTimerMin = this._settingsData.focusTime;
        } else {
            this._settingsData = {
